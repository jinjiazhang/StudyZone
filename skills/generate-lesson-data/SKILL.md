...
7. Use diverse exercise types supported by the docs and shared types.
   ...
   - English reading and sequencing: `reading_comprehension` (story/passage + sub-questions — the passage is shown in the prompt, so this is the correct way to test story content), `picture_order` (order dialogue lines or cards by conversation/temporal logic), and `true_false` for language/knowledge judgments.
   - Select English types by the lesson's strand: word-recognition lessons lean on `translate_choice`/`image_choice`/`match_pairs`; dialogue lessons on `dialogue_complete`; story lessons on `reading_comprehension` (for plot) and `picture_order` (for logical/dialogue ordering). Do not force every type into every unit.
...
- **Self-containment (critical).** Every English exercise must be answerable from English knowledge (vocabulary, grammar, sentence patterns) plus learned/general knowledge, WITHOUT the textbook open. The textbook supplies the *theme* and the words/patterns being tested — it is not a lookup source the learner consults while answering. Do NOT write standalone questions that require recalling specific story-plot facts (which character did what, which animal appeared, the exact panel order). Test story content only through `reading_comprehension`, where the passage is shown in the prompt. Festival/cultural facts that are the unit's explicit learning objective (e.g. zongzi ↔ Dragon Boat Festival) are fine, because the learner has studied them.
...
- For `dialogue_complete`, make the surrounding turns establish exactly one natural reply that is derivable from the dialogue context and the taught patterns alone — not an arbitrary story-specific line (e.g. avoid "Here's a clue!" → "What's over there?"). Distractors must be grammatical but contextually wrong. Set `blankIndex` to the `null`-text turn.
- For `reading_comprehension`, keep the passage grade-appropriate and short (a few sentences for grade 1-2). Every sub-question must be answerable from the passage with one correct option, and the whole exercise is graded all-or-nothing. This is the right place to test story/plot content, since the passage is shown.
- For `picture_order`, give the cards a single correct sequence whose order is recoverable from the cards themselves — conversation logic (greeting → question → answer → closing) or an explicit temporal/numeric order. Do NOT use arbitrary story panels whose order only someone who memorized the plot could reproduce. Use stable ids and reference real card images when using `imageUrl`.
- For `true_false`, base statements on language facts (translation, grammar), word/category facts, or learned/general knowledge — e.g. "Baozi is a drink." (false), "We eat mooncakes at the Mid-Autumn Festival." (true). Do NOT write story-plot statements such as "A cat brings the clue." that require the book open. Keep statements unambiguous (avoid "sometimes/usually") and roughly balance true/false within a lesson.
