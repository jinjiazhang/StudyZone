...
export function CourseDetail() {
  ...
  if (isLoading) {
    return <div style={muted}>载入课程内容...</div>;
  }

  if (!course) {
    return <div style={muted}>课程不存在</div>;
  }
        ...
        {course.units.map((unit) => (
          <div key={unit.id} style={{ marginBottom: 24 }}>
            ...
            {unit.skills.map((skill) => (
              <div key={skill.id} style={skillBlock}>
                <div style={skillTitle}>
                  <span style={{ fontSize: 22 }}>{skill.icon}</span>
                  <span>{skill.name}</span>
                  <span style={pill}>{skill.lessons.length} 关卡</span>
                </div>

                {skill.lessons.map((lesson) => (
                  <div key={lesson.id} style={lessonBlock}>
                    <div style={lessonTitle}>
                      Level {lesson.level} · Lesson {lesson.orderIndex + 1}
                      <span style={muted}>{lesson.exercises.length} 题</span>
                    </div>
                    <div style={{ display: 'grid', gap: 8 }}>
                      {lesson.exercises.map((exercise) => (
                        <button
                          ...
                          style={{
                            ...exerciseRow,
                            borderColor: selected?.id === exercise.id ? '#3FB984' : '#e5e7eb',
                          }}
                        >
...
function summary(prompt: unknown) {
  ...
  return String(p.source ?? p.question ?? p.statement ?? p.word ?? p.type ?? '未命名题目');
}
...
const skillTitle: React.CSSProperties = {
  display: 'flex',
  alignItems: 'center',
  gap: 10,
  padding: 12,
...
const lessonBlock: React.CSSProperties = {
  padding: 12,
