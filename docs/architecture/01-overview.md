...
| 内容编辑（B 端） | 内部教研，制作官方课程内容 | 在 CMS 后台编辑课程、关卡、题目 |
| **创作者（UGC）** | 普通用户开通创作者权限后制作课程 | 创建课程、编辑关卡、提交审核、查看学习数据与收益 |
| **审核员** | 平台内容安全岗 | 审核 UGC 提交、违规处置、申诉处理 |
...
| **Admin** | `admin/` | 后台运营接口 |
| **Creator**（计划，Phase 4） | `creator/` | UGC 创作者：草稿、提交、审核、发布、版本管理 |

> 当前阶段所有模块跑在同一个 NestJS 进程里（**模块化单体**）；未来按流量切分为微服务时，模块边界已经预留好。
>
> Phase 4 引入 UGC 后，`Creator` 与 `Content` 协作：UGC 草稿在 `Creator` 域内可写、可改；通过审核后晋升为只读的 `Content` 实体，被学习引擎消费。
...
按"先跑通主链路，再加深"的原则分多期演进。当前进度参见后续文档。

- **Phase 0** ✅ 基础设施搭建（Monorepo + Docker 本地栈 + CI 雏形）
- **Phase 1** ✅ MVP 主干：账户 / 课程 / 多种题型 / 学习引擎 / XP / Streak / Web + Mobile 双端打通
- **Phase 2** 🟢 进行中：SRS / 好友 / 周联赛 / 推送 / 数据埋点
- **Phase 3** ⏳ 计划：订阅会员 / 宝石商店 / 第二个学科 / 官方 CMS / A/B 实验
- **Phase 4** ⏳ **UGC 创作者生态**：用户制作并发布 course 内容
  - **4.1 创作者基础**
    - 创作者身份：普通用户申请开通，平台分级（新手 / 认证创作者）
    - 课程草稿：在 Web / Admin 端可视化编辑器中创建 Course → Unit → Lesson → Exercise
    - 题型复用：复用平台现有题型库（`packages/shared-types/src/exercise.ts`）
    - 媒体上传：图片 / 音频走对象存储（S3 / OSS），自动转码
  - **4.2 审核与发布**
    - 内容版本：`draft → submitted → published / rejected`，每次大改走重新审核
    - 自动初筛：敏感词、违规图片、AI 内容评估
    - 人工复审：审核员后台处理队列；可附整改意见
    - 灰度发布：先小流量验证，再全量上架
  - **4.3 学习者侧消费**
    - UGC 课程独立分区（与官方课程区分），支持搜索、分类、推荐
    - 学习数据与官方课程一致：XP / Streak / SRS / 联赛全部打通
    - 评分与举报：学习者可对课程打分、留言、举报
  - **4.4 创作者激励**
    - 数据看板：学习人数、完成率、平均评分、留存
    - 收益分成：付费课程 / 打赏 / 平台流量分成（与会员订阅打通）
    - 创作者排行榜与勋章
  - **4.5 数据与合规**
    - 内容指纹（防搬运 / 抄袭检测）
    - 版权与举报处置流程
    - 未成年人内容隔离
- **Phase 5** ⏳ 规模化：微服务拆分 / 多区域 / AI 助教（含 AI 辅助创作）/ 离线模式
